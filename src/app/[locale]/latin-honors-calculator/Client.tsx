...
import { calculateLatinHonors, SCHOOL_CONFIGS, type SchoolSystem } from "../../../core/calculators/latinHonors";
...
export default function LatinHonorsClient() {
	...
	return (
		<ToolLayout>
			...
			<div className="tool-grid" style={{ marginTop: "24px" }}>
				<div className="card">
					...
					<div className="form-group" style={{ marginBottom: "24px" }}>
						<label className="form-label" htmlFor="school">{t("schoolLabel")}</label>
						<select
							id="school"
					...
					<InteractiveSlider
						label={t("gwaLabel")}
						value={gwa}
						onChange={(val: number | string) => updateState({ gwa: Number(val) })}
				...
				<div className="card" style={{ backgroundColor: "var(--bg-color)" }}>
					...
					<div style={{ marginBottom: "24px", padding: "24px", backgroundColor: hasHonor ? "rgba(46, 125, 50, 0.05)" : "rgba(13, 71, 161, 0.05)", border: `1px solid ${hasHonor ? "#2e7d32" : "var(--primary)"}`, borderRadius: "8px", textAlign: "center" }}>
						...
						<strong style={{ display: "block", fontSize: "28px", color: hasHonor ? "#1b5e20" : "var(--text-secondary)", lineHeight: 1.2 }}>
							{hasHonor ? `🎓 ${honor}` : t("noHonor")}
						</strong>
					</div>

					{!isHighest && nextHonor && (
						<div style={{ padding: "16px", backgroundColor: "var(--surface-color)", borderLeft: "4px solid #f57c00", borderRadius: "4px", marginBottom: "20px" }}>
							<p style={{ fontSize: "14px", color: "var(--text-secondary)", margin: 0, lineHeight: 1.5 }}>
								{t("gapLabel")} <strong style={{ color: "#e65100" }}>{gap.toFixed(3)}</strong> {t("gapSuffix")} <strong>{nextHonor}</strong>
							</p>
						</div>
					)}
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
						{config.honors.map((h) => {
							...
							return (
								<div key={h.label} style={{ display: "flex", justifyContent: "space-between", padding: "10px 12px", borderRadius: "6px", backgroundColor: isActive ? "rgba(46, 125, 50, 0.08)" : "transparent", border: isActive ? "1px solid #2e7d32" : "1px solid var(--border-color)" }}>
									<span style={{ fontSize: "14px", fontWeight: isActive ? 600 : 400, color: isActive ? "#1b5e20" : "var(--text-secondary)" }}>
										{isActive && "✓ "}{h.label}
									</span>
			...
			<ToolFooter currentPath="/latin-honors-calculator" />
		</ToolLayout>
	);
}
