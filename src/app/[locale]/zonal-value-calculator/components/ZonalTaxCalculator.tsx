...
export default function ZonalTaxCalculator({ streets }: ZonalTaxCalculatorProps) {
    const [selectedStreet, setSelectedStreet] = useState<string>(streets[0].street);
    const [areaSqm, setAreaSqm] = useState<number>(100);
    const [sellingPrice, setSellingPrice] = useState<number>(5000000);

    const currentStreet = streets.find(s => s.street === selectedStreet) || streets[0];
    ...
    return (
        <div className="card tool-grid" style={{ marginTop: "24px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
            <div className="input-section">
                <h3 style={{ borderBottom: "1px solid var(--border-color)", paddingBottom: "8px", marginBottom: "16px" }}>Property Details</h3>
                
                <div style={{ marginBottom: "16px" }}>
                    <label style={{ display: "block", marginBottom: "8px", fontWeight: "bold" }}>Select Street / Classification</label>
                    <select 
                    ...
                    >
                        {streets.map(s => (
                            <option key={s.street} value={s.street}>
                                {s.street} ({s.classification}) - {formatCurrency(s.valuePerSqm)}/sqm
                            </option>
                        ))}
                    </select>
                </div>
            ...
            <div className="result-section" style={{ backgroundColor: "#f5f9ff", padding: "24px", borderRadius: "12px", border: "1px solid #d0e1fd" }}>
                <h3 style={{ marginTop: 0, color: "var(--primary)" }}>Tax Computation</h3>
                ...
                <hr style={{ borderColor: "#d0e1fd", borderStyle: "solid", marginBottom: "16px" }} />
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: "16px", paddingTop: "16px", borderTop: "2px solid var(--primary)", fontSize: "1.2rem", fontWeight: "bold", color: "var(--primary)" }}>
