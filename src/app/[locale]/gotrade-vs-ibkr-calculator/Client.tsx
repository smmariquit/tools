		...
		<ToolLayout maxWidth="1200px">
			...
			<div className="tool-grid" style={{ marginTop: "24px" }}>
				<div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
					<div className="card">
						<h2 style={{ fontSize: "18px", marginBottom: "16px", color: "var(--primary)" }}>{t("investmentDetails")}</h2>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
					<div className="card" style={{ position: "sticky", top: "100px", backgroundColor: "var(--bg-color)" }}>
						<h2 style={{ fontSize: "20px", marginBottom: "16px", color: "var(--primary)" }}>{t("comparisonResults")}</h2>
						
						<div style={{ padding: "16px", backgroundColor: "var(--bg-color)", borderRadius: "8px", marginBottom: "16px", borderLeft: "4px solid #0EA5E9" }}>
							<h3 style={{ fontSize: "14px", marginBottom: "12px", color: "#0EA5E9" }}>{t("gotradeTitle")}</h3>
							<div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px" }}><span>{t("actualUsdReceived")}</span> <strong>{formatUSD(gotradeUsdDeposited)}</strong></div>
							<div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", color: "red", marginTop: "4px" }}><span>{t("gotradeTotalLoss")}</span> <span>{formatPHP(gotradeTotalFriction)}</span></div>
						</div>

						<div style={{ padding: "16px", backgroundColor: "var(--bg-color)", borderRadius: "8px", marginBottom: "16px", borderLeft: "4px solid #D92D20" }}>
							<h3 style={{ fontSize: "14px", marginBottom: "12px", color: "#D92D20" }}>{t("ibkrTitle")}</h3>
							<div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px" }}><span>{t("actualUsdReceived")}</span> <strong>{formatUSD(ibkrUsdDeposited)}</strong></div>
							<div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px", color: "red", marginTop: "4px" }}><span>{t("ibkrTotalLoss")}</span> <span>{formatPHP(ibkrTotalFriction)}</span></div>
						</div>
