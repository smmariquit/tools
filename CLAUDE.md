...
- **Canonical policy:** Every page gets a self-referencing canonical from `src/app/components/Canonical.tsx` (client component; React hoists the `<link>` into `<head>` during SSR). NEVER hardcode per-page `alternates.canonical`. One exception: blog posts embedded on a tool page canonicalize to that tool page via `src/lib/articleCanonicals.ts` — regenerate that map (grep `ToolPageBottom slug=`) when adding or moving a tool article.
- **Locale indexing:** `/tl` and `/ceb` are noindexed via `X-Robots-Tag` in `src/proxy.ts` until their long-form articles are human-translated. The sitemap lists `/en` only. Do not add hreflang until translations are human-complete.
- **Next 16 params:** `params` is a Promise — always `await params` in `generateMetadata` and page components. Sync destructuring (`params: { locale }`) silently yields `undefined` (this once shipped live `/undefined/` canonicals on 9 pages).
